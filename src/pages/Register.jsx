...
function Register() {
  ...
  const handleSubmit = async(event) => {
    ...
    if (handleValidation()) {
      const { password, username, email } = inputValues;
  ...
  const handleValidation = () => {
    ...
    if (password !== confirmPassword) {
      toast.error("Wrong confirm password :D", toastConfig);
      return false;
    };
    ...
    if (password.length < 8) {
      toast.error("Password should be equal or greater than 8 characters", toastConfig);
      return false;
    }
    if (email === "") {
